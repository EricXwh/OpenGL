...
int main()
{
   ...
   // ------------------------------
    ...
    //shader
    Shader colorShader("./src/11-12_Multilighting/Shader/vertex.glsl", "./src/11-12_Multilighting/Shader/fragment.glsl");
    Shader lightCubeShader("./src/11-12_Multilighting/Shader/lightVertex.glsl", "./src/11-12_Multilighting/Shader/lightFragment.glsl");
   ...
   // ------------------------------------------------------------------
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)0);
    glEnableVertexAttribArray(0);
    unsigned int diffuseMap = loadTexture("asset/textures/container2.png");
    unsigned int specularMap = loadTexture("asset/textures/container2_specular.png");
    colorShader.use();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            colorShader.setVec3(base + "position", pointLightPositions[i]);
            colorShader.setVec3(base + "ambient", 0.05f, 0.05f, 0.05f);
            colorShader.setVec3(base + "diffuse", 0.8f, 0.8f, 0.8f);
            colorShader.setVec3(base + "specular", 1.0f, 1.0f, 1.0f);
            colorShader.setFloat(base + "constant", 1.0f);
            colorShader.setFloat(base + "linear", 0.09f);
            colorShader.setFloat(base + "quadratic", 0.032f);
        }
